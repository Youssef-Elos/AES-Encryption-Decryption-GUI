import tkinter as tk
from tkinter import messagebox
import os

# AES S-box
sbox = [
    
]

# Inverse S-box
inv_sbox = [
    
]

# Rcon (Round Constant) table
Rcon = [
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00
]

def sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = sbox[state[i][j]]
    return state

def inv_sub_bytes(state):
    for i in range(4):
        for j in range(4):
            state[i][j] = inv_sbox[state[i][j]]
    return state

def shift_rows(state):
    state[1] = state[1][1:] + state[1][:1]
    state[2] = state[2][2:] + state[2][:2]
    state[3] = state[3][3:] + state[3][:3]
    return state

def inv_shift_rows(state):
    state[1] = state[1][3:] + state[1][:3]
    state[2] = state[2][2:] + state[2][:2]
    state[3] = state[3][1:] + state[3][:1]
    return state

def mix_columns(state):
    
    for i in range(4):
        col = [state[j][i] for j in range(4)]
        col = [col[0] ^ col[1] ^ col[2], col[1] ^ col[2] ^ col[3], 
               col[2] ^ col[3] ^ col[0], col[3] ^ col[0] ^ col[1]]
        for j in range(4):
            state[j][i] = col[j]
    return state

def inv_mix_columns(state):
    
    return mix_columns(mix_columns(mix_columns(state)))

def add_round_key(state, round_key):
    for i in range(4):
        for j in range(4):
            state[i][j] ^= round_key[i][j]
    return state

def key_expansion(key):
    
    key_words = [key[i:i+4] for i in range(0, len(key), 4)]
    for i in range(4, 44):
        temp = key_words[i-1]
        if i % 4 == 0:
            temp = [sbox[b] for b in temp[1:] + temp[:1]]
            temp[0] ^= Rcon[i//4]
        key_words.append([a ^ b for a, b in zip(key_words[i-4], temp)])
    return [word for words in key_words for word in words]

class AES:
    def __init__(self, key):
        self.round_keys = key_expansion(key)

    def encrypt(self, plaintext):
        state = [[plaintext[i+j*4] for j in range(4)] for i in range(4)]
        state = add_round_key(state, [self.round_keys[i:i+4] for i in range(0, 16, 4)])
        
        for round in range(1, 10):
            state = sub_bytes(state)
            state = shift_rows(state)
            state = mix_columns(state)
            state = add_round_key(state, [self.round_keys[round*16+i:round*16+i+4] for i in range(0, 16, 4)])
        
        state = sub_bytes(state)
        state = shift_rows(state)
        state = add_round_key(state, [self.round_keys[160+i:160+i+4] for i in range(0, 16, 4)])
        
        return [state[i][j] for j in range(4) for i in range(4)]

    def decrypt(self, ciphertext):
        state = [[ciphertext[i+j*4] for j in range(4)] for i in range(4)]
        state = add_round_key(state, [self.round_keys[160+i:160+i+4] for i in range(0, 16, 4)])
        
        for round in range(9, 0, -1):
            state = inv_shift_rows(state)
            state = inv_sub_bytes(state)
            state = add_round_key(state, [self.round_keys[round*16+i:round*16+i+4] for i in range(0, 16, 4)])
            state = inv_mix_columns(state)
        
        state = inv_shift_rows(state)
        state = inv_sub_bytes(state)
        state = add_round_key(state, [self.round_keys[i:i+4] for i in range(0, 16, 4)])
        
        return [state[i][j] for j in range(4) for i in range(4)]

    def generate_random_key():
        return os.urandom(16).hex()

class AESApp:
    def __init__(self, master):
        self.master = master
        master.title("Custom AES Encryption/Decryption")

        self.label = tk.Label(master, text="Enter text (16 bytes, hex format):")
        self.label.pack()

        self.text_entry = tk.Entry(master, width=50)
        self.text_entry.pack()

        self.key_label = tk.Label(master, text="Enter key (16 bytes, hex format):")
        self.key_label.pack()

        self.key_entry = tk.Entry(master, width=50)
        self.key_entry.pack()

        # Add a frame for key-related buttons
        self.key_button_frame = tk.Frame(master)
        self.key_button_frame.pack()

        # Add the Generate Key button
        self.generate_key_button = tk.Button(self.key_button_frame, text="Generate Key", command=self.generate_key)
        self.generate_key_button.pack(side=tk.LEFT, padx=5)

        # Add a Clear Key button
        self.clear_key_button = tk.Button(self.key_button_frame, text="Clear Key", command=self.clear_key)
        self.clear_key_button.pack(side=tk.LEFT, padx=5)

        self.encrypt_button = tk.Button(master, text="Encrypt", command=self.encrypt)
        self.encrypt_button.pack()

        self.decrypt_button = tk.Button(master, text="Decrypt", command=self.decrypt)
        self.decrypt_button.pack()

        self.result_label = tk.Label(master, text="Result:")
        self.result_label.pack()

        self.result_text = tk.Entry(master, width=50)
        self.result_text.pack()


    def generate_key(self):
        key = AES.generate_random_key()
        self.key_entry.delete(0, tk.END)
        self.key_entry.insert(0, key)

    def clear_key(self):
        self.key_entry.delete(0, tk.END)

        
    def encrypt(self):
        text = self.text_entry.get().strip()
        key = self.key_entry.get().strip()

        try:
            text_bytes = bytes.fromhex(text)
            key_bytes = bytes.fromhex(key)

            if len(text_bytes) != 16 or len(key_bytes) != 16:
                raise ValueError("Input and key must be 16 bytes (32 hex characters) each")

            aes = AES(key_bytes)
            encrypted = aes.encrypt(text_bytes)
            self.result_text.delete(0, tk.END)
            self.result_text.insert(0, encrypted.hex())
        except Exception as e:
            messagebox.showerror("Error", str(e))

    def decrypt(self):
        text = self.text_entry.get().strip()
        key = self.key_entry.get().strip()

        try:
            text_bytes = bytes.fromhex(text)
            key_bytes = bytes.fromhex(key)

            if len(text_bytes) != 16 or len(key_bytes) != 16:
                raise ValueError("Input and key must be 16 bytes (32 hex characters) each")

            aes = AES(key_bytes)
            decrypted = aes.decrypt(text_bytes)
            self.result_text.delete(0, tk.END)
            self.result_text.insert(0, decrypted.hex())
        except Exception as e:
            messagebox.showerror("Error", str(e))

root = tk.Tk()
app = AESApp(root)
root.mainloop()
